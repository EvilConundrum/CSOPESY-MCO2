...
public:
    ...
    /**
     * Records when a process is terminated
     */
    void recordProcessTermination(std::shared_ptr<Process> process)
    {
        ...
        // we wont delete from running process so we can screen -r into the process
        ...
        else
        {
            for (const auto &process : runningProcesses)
            {
                report << process->getPID() << "   ("
                       ...
                       << "Core: " << (runningProcesses.size() <= numCpuCores ? std::distance(runningProcesses.begin(), std::find(runningProcesses.begin(), runningProcesses.end(), process)) : 0)
